...
its glyphs. The syntax module and CLI run on Node.js 22 or newer.
...
<script type="module">
  import {
    renderDiagram,
  } from "https://cdn.jsdelivr.net/npm/lines-and-arrows@1.0.0/+esm";
...
| --- | --- |
| `mode` | `view` by default. Use `edit` to enable the visual editor, undo and redo controls, and editing keyboard shortcuts. Editing also works from the keyboard alone: Tab follows the diagram layout, and each timeline insertion mark offers Message, Group, and Gap. The editor never grows past its natural width and shrinks with its container until it reaches 720 pixels wide or 60% of its natural width, whichever is larger; below that, for example on a phone, it scrolls horizontally so its editing controls stay usable. |
